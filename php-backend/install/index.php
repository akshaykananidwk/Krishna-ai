...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$locked) {
    ...
    $apiKey   = trim($_POST['anthropic_key'] ?? '');
    $model    = trim($_POST['anthropic_model'] ?? '') ?: 'claude-opus-4-8';
...
<?php else: ?>
  ...
  <form method="post" class="card" autocomplete="off">
    ...
    <label>Anthropic API key <small>(used for chat in Part 2 — you can leave blank now)</small></label>
    <input name="anthropic_key" value="<?= htmlspecialchars($_POST['anthropic_key'] ?? '') ?>">

    <label>Anthropic model</label>
    <input name="anthropic_model" value="<?= htmlspecialchars($_POST['anthropic_model'] ?? 'claude-opus-4-8') ?>">
